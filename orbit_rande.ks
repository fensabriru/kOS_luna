SET perigee TO 420000.		//Desired orbit (periapsis)
...
else
{
	switch to 0.
	run once rendezvous.
	if TimeToLaunchWindow(GetOrbitNormal(target), 28) > 300
	{
		warpto(time:seconds + TimeToLaunchWindow(GetOrbitNormal(target), 28) - 300).
	}
	wait until TimeToLaunchWindow(GetOrbitNormal(target), 28) < 290.
